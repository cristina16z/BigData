...
/*Resultado = 3772 documentos*/ 
use('BigData');
db.restaurants.find({},{restaurant_id:1, name:1, cuisine:1, borough:1}).count();
//select restaurant_id, cuisine, borough, name from restaurants
...
use('BigData');
db.restaurants.find({borough:"Bronx"},{_id:0, restaurant_id:1, name:1, cuisine:1, borough:1});
...
use('BigData');
db.restaurants.find({"grades.score": {$gt:90}},{grades:1});
...
/*Resultado 3 documentos)*/
use('BigData');
db.restaurants.find({grades:{$elemMatch:{score: {$gt:80, $lt:100}}}}).count();
